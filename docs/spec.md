...
| A module that defines a name (a `def`, a `class`) and LATER imports the same name is refused with the fix named: Python's later binding wins, a Rust module holds one item per name, and dropping the definition would lose what ran between the two. The mirror order — an import, then a definition of the name — drops the dead import with a warning; the definition is the binding (the exception closure and the import lowering agree) | Loud (Devin review on #336/#338) |
