...
| A SHARED class's `==`/ordering with a REFLECTED comparison (other.__eq__(self)) when the first __eq__/__lt__ returns NotImplemented is not modeled: the NotImplemented return in `__eq__` lowers to the identity test (the same fallback CPython ends at when the reflected `__eq__` declines too), and ref_cmp's Greater case calls the RIGHT operand's __lt__ (a pure total-order __lt__ makes the two calls observationally identical to CPython's sort) | Model limit of the shared-equality round (#329) |
...
| `return NotImplemented` in a METHOD named `__eq__` is CPython's identity fallback, decided by the other parameter's lowering: a shared class's `object`/own-class other is the PyRef class, so the fallback is the two borrows' address (`t == t` True, two instances False); a boxed `object` other of a value class is never the same object (`false`); a value class compared with its own kind has no identity to ask — a loud rustc error. In any other function (a free function named `__eq__` included) it is a loud rustc error at the site (`compile_error!` naming the construct — `__ne__`'s identity default and an ordering or arithmetic dunder's reflected attempt are not modeled) | Correct-or-loud (the evaluation on issue #137) |
