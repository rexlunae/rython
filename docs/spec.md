...
| `math.cbrt` hands off to the platform libm, which is NOT correctly rounded on every system: glibc returns `cbrt(27.0) == 3.0000000000000004` while Apple's libm returns exactly `3.0` (both faithful-rounded, within 1 ulp). rython calls the same libm as CPython on each ABI, so it matches CPython exactly per platform; a single byte-exact pin cannot hold across platforms, so the transcript and runtime pin use input roots exact everywhere (`-8.0` → `-2.0`, `0.0` → `0.0`) and the `cbrt(27.0)` case is pinned within IEEE tolerance. The other `Into<f64>` math scalars (`fma`, `hypot`, `nextafter`) are platform-identical | Platform (libm) difference, not a rython defect; CPython-exact on each ABI |
