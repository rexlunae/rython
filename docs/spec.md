...
| A module's body runs where Python runs it — at the IMPORT SITE, once: every crate module's `__module_init__` is once-guarded (the guard is taken on entry), and an import of a crate module — at module level (in the importer's init, in statement order) or inside a function (at the call site, on first execution) — calls the loaded modules' inits in Python's order (`import a.b`: the package `a`, then `a.b`; `from .a import x`: `a`, then `x` when it is a submodule). A module imported only inside a never-called function never runs; an import cycle (`a` imports `b` imports `a`) finds `a`'s init already entered and continues with `a`'s partial state, as CPython's partially-initialized module does. The entry's `main` runs its own init. The package root `__init__` is the lib root, which the bin crate does not contain: what it runs at import time (statements, the crate modules its imports load, computed assignments) does not run for the binary, and the converter warns | Correct (issue #333, Devin review on #336); root `__init__`: loud |
| A module-level binding is FLOW-SENSITIVE for the exception closure: a class definition after a crate import of the same name rebinds the name to the class, and a crate import after a class definition rebinds it to the imported class — the straight line's last binding is what a base resolves to (the Rust output of such a module still carries both items, which rustc rejects — loud) | Correct (Devin review on #336) |
