...
quoting diverges, §4.6); `ValueError` from `int()` and `float()` on a
string (CPython's messages; Python's numeric-string grammar — single
underscores between digits only — is one runtime authority shared with
argparse's `type=int`/`type=float`), `chr()`, and `math` domain errors;
...
| `math.cbrt` calls the Rust `libm` crate — a fixed software cube root, correctly rounded for perfect cubes (`cbrt(-8.0)` = `-2.0`, `cbrt(27.0)` = `3.0`) and deterministic on every ABI. CPython instead calls the PLATFORM libm, which is not correctly rounded on every system: glibc returns `cbrt(27.0) == 3.0000000000000004` while Apple's libm returns exactly `3.0` (both within 1 ulp). So rython matches CPython exactly only where the platform libm's cube root is the exact root (e.g. `-8.0`/`0.0`); where CPython's libm is off by an ulp (`27.0` under glibc) rython's correctly-rounded value differs, within IEEE tolerance. The transcript and runtime pin use input roots exact under every libm, and `cbrt(27.0)` is pinned within tolerance. `fma`, `hypot`, `nextafter` are platform-identical | Model limit: a fixed correctly-rounded software `cbrt` vs CPython's platform-libm `cbrt` (agree within 1 ulp; bit-identical for perfect cubes) |
| `math.lgamma` / non-integer `math.gamma` are platform-divergent like `cbrt`: `lgamma` (and thus `gamma(0.5)`, `gamma(-2.5)`, ...) follow CPython's PLATFORM `lgamma` and differ by ~1 ulp across libms (macOS vs glibc), while the gamma of a positive INTEGER (24.0, 120.0, 2.0, 1.0) is exact everywhere and byte-identical | Model limit (the `cbrt` family); pinned within tolerance |
...
| A dict iteration whose SIZE changes mid-loop (a key added or removed while `for k in d` runs) does not raise CPython's `RuntimeError`: the iteration lowers to a `py_keys()` Vec snapshot (the established dict-views model — `keys()`/`values()`/`items()` return Vecs), so the loop sees the pre-mutation keys. Updates to EXISTING values are allowed, matching CPython | Model limit; the dict-views snapshot model (the review's fix-6 round) |
...
| The exception-attribute model binds every raise argument other than a bare name or a constant to a typed temporary once, in source order (a property read or a call runs once); the message and each attr read the temporary | Correct; CPython's single evaluation (the evaluation on issue #137; Devin review on #330) |
| An exception __init__ with a statement beyond the `self.<field> = <param>` stores, the `super().__init__(<message>)` call, a docstring, or `pass` is a loud rustc error at the raise site (`compile_error!` naming the class and line — the statement would not run there); a keyword-only parameter with a default is modeled as its default when the raise passes no keywords, and a message reading a stored field (`super().__init__(self.message)`) means the parameter it stores | Correct-or-loud (the evaluation on issue #137) |
| The message is `str(e)` as CPython sets it: the `super().__init__(<message>)` argument; the empty string for `super().__init__()` (the fields are kept); with no super call at all, the one positional argument of the raise, or empty for none (`BaseException.__new__` records the call's arguments). A super call with two or more arguments is the args tuple's repr; no super call with two or more positional arguments, and a message holding a lambda or a comprehension (bindings of its own the raise-site rewrite cannot model), are loud rustc errors at the raise site | Correct-or-loud (Devin review on #330) |
| A zero-parameter `__init__(self)` is modeled like any other (a fixed message; an argument at the raise is the arity error); a variadic `__init__(self, *args, **kwargs)` is modeled when its body only forwards to `super().__init__(*args)` (idna's IDNAError), the message being the raise's one positional argument or empty; named parameters before `*args` (`__init__(self, prefix, *args)`) take the first arguments, evaluated once and dropped, and the forwarded slice is the rest — a body doing more, a field store of a named parameter, a keyword-only parameter, a starred call that is not its own `*args`/`**kwargs`, or a keyword are loud. Every argument of a raise is evaluated once in source order, a bare name bound to a parameter the model ignores included (`raise E(undefined_name)` is loud in rustc as CPython's NameError is). The last `self.<field> = <param>` store of a field wins for the attrs, as in Python, while a `self.<field>` read in the message means the store as it stood AT the `super().__init__` call; a name the message reads at the raise site (a module global, a builtin) must be bound there exactly as the `__init__` sees it — a caller local or another module's same-named global is a loud rustc error at the site; a stored field whose argument is a class instance (`raise EmptyPoolError(self, ...)` — urllib3) is a loud rustc error at the site too, since the boxed attrs cannot hold one; a second `super().__init__` call (which would replace the args) is loud. A bare-name argument is captured, in source order, before a LATER argument that runs code (`E(counter, bump())` reads counter before bump rebinds it) | Correct-or-loud (Devin review on #330) |
| The exception closure is keyed by bare class name (the hierarchy index's rule): a name two modules both define is ambiguous and stays out of it as a class and as a base — each definition is judged by its own name and direct bases, so an ordinary `Node` is never erased by an exception `Node(Root)` elsewhere — with a definition warning when the exclusion loses a transitive exception | Loud limit (Devin review on #330) |
| An exception class's ancestors are its Python MRO (C3 linearization over every base, left to right: `class Both(A, B)` is caught by `except B:` and by B's builtin ancestors, and `issubclass` sees both branches); the `__init__` a construction runs and the field a handler reads resolve along the same order. A builtin base takes its full interpreter MRO into the merge, so `class C(Exception, ValueError)` is refused as CPython refuses it while `class D(ValueError, Exception)` records `ValueError, Exception`; a base the model cannot place (an ordinary class, an unresolved import, an ambiguous alias, an unknown dotted name) is loud, never dropped. A hierarchy with no consistent MRO (`C(A, B)` with `B(A)` — CPython's TypeError at class creation) is a conversion error at the definition, and so is a cyclic chain (`class B(A)` then `A = B`, where the name-keyed resolution turns back on itself) | Correct-or-loud (Devin review on #330) |
| A program binding a name under the `__rython_` prefix (a local, a parameter, a def, a loop target, a walrus, an import, an `except ... as`) is a conversion error at the module: the prefix is reserved for the temporaries the conversion emits (a bound receiver, a captured argument, a loaded value), which would otherwise shadow the binding silently | Loud (Devin review on #331) |
| `return NotImplemented` names the singleton only when no parameter or local of the function shadows the name; a shadowing binding is an ordinary value. A `global NotImplemented` declaration alone redirects the lookup to the module scope, where the builtin still answers unless the module (or the body, through the declaration) binds the name. An explicit `obj.__eq__(other)` call on a shared class whose `__eq__` can decline is a loud error at the call (the singleton has no runtime value; `==` runs the reflected `__eq__` and identity as CPython does) | Correct-or-loud (Devin review on #330) |
| Exception-ness is TRANSITIVE through the crate's emitted classes (`Root(Exception)`, `Mid(Root)`, `LeafError(Mid)`: `Mid` is an exception class whatever its name — a marker, no trait machinery — and LeafError's ancestor chain records Mid, so `except Mid:` catches it); the closure is one crate-wide index — keyed by (module, class), resolved module by module (the module-qualified row below) — installed like the hierarchy roots | Correct (Devin review on #330) |
| A base, a handler, or an `issubclass` operand named through an ALIAS (`R = Root`, `from errors import Root as R`, `Base = ValueError`) is its canonical class: the ancestor chain records `Root`/`ValueError`, `except R:` matches `Root`, and a module-level alias of an exception — bound ONCE in the module's straight line; a name bound to a class ONLY inside a `try:`, an `if`, a loop, or a `with` the gates cannot fold may be that class or unbound at runtime, so it is runtime-decided like a two-way alias (the conversion does not prove a body executes); an import alias only from a module of the crate, never an external package's same-named class (a name a module binds by an EXTERNAL import — `from somewhere_else import Root`, then `R = Root` or `class Node(Root)` — is that external object in that module, never the crate's `Root` nor a builtin; every module-level binding takes part in the same flow — a later top-level `class Root(Exception)` REPLACES the external binding, and one under a gate is one more runtime alternative, loud — and an alias resolves through its OWN module's bindings only, never another module's same-named binding; an external import's `as` binding whose bare name a local class shadows — `from http.client import IncompleteRead as httplib_IncompleteRead` beside the crate's own `IncompleteRead` — is the external class as a base or a handler, recorded under the alias spelling with a definition warning, never the local class and never a false cycle; a base is judged by the bindings of the module DEFINING the class, whichever module raises or catches it, and a crate module's re-import of a name its source module binds externally — `from .compat import JSONDecodeError as CompatJSONDecodeError` over compat.py's `from json import JSONDecodeError` — is external in the importing module too; a crate import's `as` name whose bare name the importing module binds to a class of its own — `from urllib3.exceptions import HTTPError as BaseHTTPError` in requests' exceptions.py, which defines `HTTPError` — is the SOURCE module's class, which takes its place in the chain under the alias spelling with its own ancestors, so `except BaseHTTPError:` catches a subclass and the local `except HTTPError:` does not, as in Python, with a definition warning: the runtime matches by bare name, so the spelling is what tells the two classes apart) — makes its subclasses exception classes; a straight-line rebinding (`R = Exception` then `R = Root`) is the final binding, and a top-level binding after a gated one replaces the gated alternatives, as CPython's module init leaves it; only a binding to a class (a crate class, a builtin exception, or an alias of one) is an alias here — an ordinary value bound two ways under a gate is a plain runtime variable; an alias a module binds to more than one class across control-flow alternatives the static gates cannot fold (`try: R = Root / except: R = Exception`) is decided at runtime, so rython follows neither (the ambiguity is scoped to that module — an unrelated module's `R` is its own — and transitive: a copy `S = R` is decided at runtime too): a base, a raise, a handler, an isinstance, or an issubclass naming it is a loud error (a definition warning names it). An `isinstance` target through an alias matches the canonical class. A construction of an in-crate exception class is ONE lowering wherever the call sits (a raise, an isinstance probe, a value position such as `err = MyError("x")`): the modeled `__init__`, else the generic message with the ancestors — `str(e)` as CPython sets it: the one argument, empty for none, and for two or more the args tuple's repr (`(<Pool object>, 'Pool is closed.')`, each argument's repr; the default object repr drops the address, §12.3); the same rule for a modeled `super().__init__(a, b)` and for a forwarded `*args` slice. Every construction records its arguments' reprs, so `repr(e)` is `Kind(repr(a), repr(b))` and an exception passed as an argument to another keeps its structure (`(Inner('a', 'b'), 'x')`) (a builtin exception name in value position is its name string, like a crate class). `raise R` / `raise R(...)` / `isinstance(R(...), Root)` under `R = Root` construct a `Root`. A class without its own `__init__` constructs through the first one up its chain (`class Child(Base): pass` runs Base's — the kind stays Child, the ancestors Child's, the field reads resolve through the chain). A `super().__init__(...)` call whose next initializer up the MRO is USER-DEFINED runs that initializer at the call, with the call's arguments (positionals, keywords, constant defaults) bound to its parameters — its message, its stores, its own super call, down to `BaseException.__init__` (which takes no keyword: loud); the attrs are every level's stores in execution order (a child's store before its super call is overwritten by the base's, a store after it overwrites the base's), and a field read's type follows the same order. Every expression the construction evaluates — an argument of the call or of any super call up the chain — that may run code (a call, a property read, a subscript, a walrus) is bound ONCE, in evaluation order, and read by the message, the attrs, and the recorded repr; an argument that reads a name — bare, or inside a pure combinator (`counter + 1`) — is captured (evaluated, cloned) first when a later expression of the construction may run code (`raise Ticket(counter)` whose message calls `bump()` records the value at the call); an argument the initializer never reads is still evaluated once, in source order (`raise Ignore(1 // 0)` is the ZeroDivisionError, a bare name is read once). A user class over `BaseException` is caught by `except BaseException:` and NOT by `except Exception:` — the recorded chain decides; only a chain with no builtin in it keeps the broad posture | Correct (Devin review on #330) |
| The exception closure is MODULE-QUALIFIED: a base resolves to the class its own module binds — the module's own class of that name, or the class a crate import brings in, followed through re-exports to its defining module — so `ReadTimeout(Timeout)` in requests' exceptions.py derives from requests' exception `Timeout` while urllib3's ordinary `Timeout` is a different class. The bare-name REGISTRY the class emission consults still holds only names one module defines (the runtime matches exception classes by bare name, so two same-named classes cannot be told apart there); a twice-defined class is judged by its own name and direct bases, with the definition warning when that loses a transitive base | Correct (issue #333) |
| A function LOCAL or PARAMETER that shares its name with a module STATIC of the generated crate (a constant or promoted module value, a mutable global, a `use`-bound import) is spelled apart as `__rython_local_<name>` throughout the def — its parameter list, its body, and nested scopes that capture it, never one that binds the name itself; a name the def declares `global` is the static. rustc forbids a binding that shadows a static (E0530), and the module's own reads of the static must not see the local (requests' compat.py) | Correct (issue #333) |
| A module value stored TWICE OR MORE (anywhere in the module body — under an import guard, an `if`, a `try`) and read by a function is a MUTABLE static: the first top-level store is its initializer (a `LazyLock<Mutex<T>>`, boxed when no type infers), a value with no top-level store a `Mutex<PyValue>` holding None, every other store a write-through; the promotion authority sees the module body with its import guards folded exactly as the emission does. A class-valued store (`BaseSSLError = ssl.SSLError`) or a declaration-shaped one is a compile-time binding, never such a static; a definite if/else or try/except pair stays the LazyLock promotion (requests' adapters.py `_preloaded_ssl_context`) | Correct (issue #333) |
| A single-store alias of a stdpython-module constant (`_ver = sys.version_info`) is a `pub use ... as name` item and nothing else — the init hoisting never declares the same name as a local (E0530) | Correct (issue #333) |
| A synthesized definition rendered in ANOTHER module (the `lru_cache(maxsize=N)(f)` wrapper) expands every TYPE-ALIAS name of the wrapped function's return annotation to the alias's own type in the module that binds it (`-> CoherenceMatches` of charset_normalizer's cd.py wrapped in api.py); a `TypedDict` class in an annotation is the boxed-value dict (`PyDict<String, PyValue>` — a TypedDict is a dict), and a class that is only a `TYPE_CHECKING` stub of its own module is the boxed value, like an imported stub | Correct (issue #333) |
| An `as` binding of an EXTERNAL import whose bare name a crate module also binds (`from cryptography import __version__ as cryptography_version` beside the crate's own `__version__`) reads as the boxed None with the external-module divergence warning, like every external import; a stdlib module's EXCEPTION CLASS read as a VALUE (`BaseSSLError = ssl.SSLError`) is the class's name string, like a bare `ValueError` in value position | Correct (external-module divergence; issue #333) |
| A module's body runs where Python runs it — at the IMPORT SITE, once: every crate module's `__module_init__` is guarded by Python's per-module import lock (the runtime's `ModuleInitLock`: NOT STARTED → RUNNING (owner thread) → DONE). An import of a crate module — at module level (in the importer's init, in statement order), under module-level control flow or in the `__main__` block (the `use` is hoisted to module scope; the init calls sit where the statement runs), or inside a function (at the call site, on first execution) — calls the loaded modules' inits in Python's order (`import a.b`: the package `a`, then `a.b`; `from .a import x`: `a`, then `x` when it is a submodule). The owning thread re-entering (an import cycle) returns at once and continues with the module's partial state, as CPython's partially-initialized module; another thread blocks (a condition variable) until the body has finished; a wait cycle across threads (thread A owns `a` and waits for `b`, thread B owns `b` and asks for `a`) lets the later importer proceed with the partial module, CPython's `_DeadlockError` path; a body that raises — or unwinds — leaves the module NOT STARTED, so a later import runs it again (CPython drops a failed import) — unless the module holds a static the process cannot re-initialize — a promoted value the body initialized, or a mutable global (a `global`-written value, which a function the body called may have written): then the module stays FAILED and every later import raises the same exception (an ImportError naming the reason when the body unwound), where CPython would run the body again with fresh globals (loud). `from package import name` takes the package's own binding of `name` (a def, a class, a store, an import alias, a name a `from m import *` of a crate module exports — `m`'s literal `__all__`, else its public module-scope names; the glob re-export honours that `__all__` too — `__all__` being the LATEST effective binding when the body has run (a later top-level literal replaces a computed or conditional one; a later membership-changing mutation — `.append`, `+=`, a subscript store, the list handed to a callee the conversion does not know, by position or by keyword, any other use that may alias it (`exports = __all__`, a container holding it), a builtin the module shadows, a def naming it — or a conditional binding makes the exports unknown, while a read the analysis sees through — `.copy()`, `len(__all__)`, `.sort()`, an `in` test, a subscript read, a loop over it — leaves it in force, and a top-level `del __all__` is back to the public names), the public names following source order too (a top-level `del x` removes `x` and the re-export is then explicit, since the deleted static outlives the name; a `del` under a module-level condition makes the exports unknown and a star import of that module is refused) — while a star import whose exports the conversion cannot enumerate (an external module, an unknown `__all__`) beside a submodule of the requested name is refused, and a `from m import *` of a module that reaches the importer is refused too (when `m` starts first the star import runs while `m` is partial and Python copies only the names bound by then, which the static glob cannot represent — unless the importer is `m`'s own package, which Python initializes before any submodule, the `__init__` idiom); a name a sibling reads through such a re-export is the defining module's static; a bare annotation binds nothing, and nothing under `if TYPE_CHECKING:` does) when it has one and loads the submodule `package.name` only otherwise, as Python's from-list does; a binding under a module-level condition still wins, and the converter says so through -W when the submodule exists (Python decides at runtime). During an import cycle the importer reads a name only once the statement binding it has run: each module keeps one bit per binding — per (statement, name) pair (a def, a class, a store, an import alias, a walrus — under module-level control flow too; the visitor's one binding enumeration), set where that binding happens (a store's target after the statement's init code; a loop target at the top of the body, so an empty iterable binds nothing; a `with` item's target right after its context expression, before the next item's; a walrus right after its store, so a later operand of the same expression sees it, and only that name), over the module's normalized body — the sequence the emission lowers after the singledispatch desugar, the import-guard fold and the gate splice, numbered the same way by the importer — and `from .a import X` while `a`'s body is still running on this thread (a self-import included) raises Python's ImportError (`cannot import name 'X' from partially initialized module 'pkg.a' (most likely due to a circular import)`) rather than handing out the static's eventual value — the check reads every statement binding `X`, so any one that has run answers; once the body has completed every name answers as bound (an untaken branch's name is the hoisted static — the static-import divergence the -W warning names); an attribute read `a.X` on the partial module is not checked (it would raise AttributeError in Python), and an `except ImportError:` handler around the import — the static import-guard idiom, which the converter folds — does not catch it: the program fails with an ImportError naming the guard and the folded fallback, loud (Python would run the fallback); a bare `except:` guard folds the same way and its site is loud for EVERY exception the module's body raises (a bare handler would have caught any of them), while a typed guard passes the other exceptions through as Python does. When the package binds the name AND a submodule of that name exists, the import site runs the submodule's body exactly when the package's binding has not happened yet (a cycle), as Python imports the submodule then; the Rust binding of the name is the package's static either way. A name the package binds AND deletes (`del x` — a no-op at module level, so the static outlives the binding) cannot be imported by a sibling: the conversion is refused, since Python decides at runtime whether the attribute, a submodule of that name, or an ImportError answers. Likewise a name a module imports, then — before redefining it (the import is dropped; the definition is the module's one item) — imports another module through (an import statement, or a call to a local or imported function whose body imports it — each called name resolved by the binding in force at the call, Python's later-binding-wins, a binding under control flow an alternative beside the last unconditional one; a call the tracing cannot see into — a method on an object, a stored value or a class called, a call of a call, a free name that is no inert builtin — counts as reaching, while an inert builtin or an external module's function loads nothing), cannot be imported by that module: Python would expose the imported value there, so the conversion is refused; a rebinding before the cycle's import, or a sibling outside the cycle, gets the definition on both sides. A name a module binds only as an `except ... as name` handler alias — bound for the handler's body, deleted when it ends — cannot be imported by a module that module reaches (the handler imports a sibling that imports the alias back): Python hands out the exception inside that window and nothing (or a submodule of that name) after it, a bind-then-unbind the bound bits cannot time, so the conversion is refused. The package root is one such module: a sibling's `from . import name` runs the root's body (once, through `crate::__rython_root`, which both crates answer) and checks its binding the same way; an absolute `from pkg import name` inside the package does not resolve to the root and takes the static as written. The no_std tier's guard is an `AtomicU8` under that tier's single-thread model (its stdpython has no threads). A module imported only inside a never-called function never runs. A name imported under a module-level condition is bound whether or not the branch runs (Python's NameError on the untaken path becomes a resolved name) and the converter says so through -W. The package root `__init__`'s body joins the binary as the `__rython_root` module, whose init the entry's `main` runs first — `python -m pkg.cli` runs `pkg/__init__.py` before `cli` — and whose items the bin root re-exports | Correct (issue #333, Devin reviews on #336/#338); conditional import binding: static-import divergence, loud |
| A module that defines a name (a `def`, a `class`) and LATER imports the same name is refused with the fix named: Python's later binding wins, a Rust module holds one item per name, and dropping the definition would lose what ran between the two. The mirror order — an import, then an UNCONDITIONAL rebinding of the name (a definition, a store — a tuple pattern, an augmented assignment, a `with` target, a walrus in the statement's own expressions, an external import: every form the one binding enumeration recognizes) — drops the dead import with a warning; the later binding is the name (the exception closure's bindings and the import lowering agree, so a class deriving from a rebound name is judged by what the name is now — a non-class base is refused). A rebinding under control flow — a loop target included (the loop over nothing leaves the import), and a walrus that may not run (a short-circuited operand, a conditional expression's branch, a comprehension) — is one of the name's runtime alternatives and leaves the import in place. A store, then an UNCONDITIONAL definition of the same name (`Root = 1` then `class Root(Exception)`) is refused the same way as the def-then-import order: one item per name, and the store may have been read between the two; a walrus that may not run before the definition (`flag and (X := 1)` then `class X`) is refused naming that shape — its store needs the name's value slot either way | Loud (Devin review on #336/#338) |
| A module-level static's FALLIBLE initializer is unwrapped inside its `LazyLock` closure whatever the shape: a trailing `?` (the call itself fails) matches on the call's Result; a `?` deeper in the expression (`compute() + 1`, `f(g())`) runs the whole expression in a Result-returning closure. Failure panics with the module-level-initialization message (§12.2) | Correct (issue #333) |
| argparse (conversion-time parsers): `type=FileType(mode)` with a literal mode opens each named file at parse time — a `PyFile` field (a mode containing 'b': the bytes file), each path opened ONCE — the parse-time open that validates it is the namespace's handle — and `-` the LIVE standard stream exactly as `FileType.__call__` picks it (stdin for a mode containing `r`, stdout for one containing `w`, `a` or `x`, text or binary — ONE object per stream PROCESS-WIDE, as `sys.stdin`/`sys.stdout` are: reads and writes go through the process's own stdin buffer and stdout handle (no per-handle read-ahead), the closed state is shared by the text and the binary handle on every thread AND by the builtins — `print()` after closing the `-` stdout and `input()` after closing the `-` stdin are CPython's closed-file ValueError (a promptless `input()` writes nothing and reads on with stdout closed, as CPython's does; one with a prompt is the ValueError), and a write stdout rejects — a closed pipe — is CPython's `BrokenPipeError: [Errno 32] Broken pipe`, raised by `print()` and by `input()`'s prompt and swallowed by the parser's own usage/help/version output as `_print_message` does (a closed stdout, though, is the ValueError `_print_message` does not ignore, escaping as CPython's traceback); every post-open failure on a disk-backed text or binary stream is the OSError subclass CPython raises for the errno (`[Errno 28] No space left on device` from a flush to /dev/full), through one mapping; a text read decodes 8192-byte chunks as TextIOWrapper does, and invalid UTF-8 is CPython's `UnicodeDecodeError` with its message (`'utf-8' codec can't decode byte 0xff in position 2: invalid start byte`; a run: `bytes in position 0-1: unexpected end of data`), positions relative to the pending tail plus the chunk, so `readline()` on a small file fails on its first call as CPython's does; the closed state has one reset, `reopen_standard_streams()`, for embedders running several programs in one process (CPython has none — a program rebinds `sys.stdout`); close() flushes it and leaves the descriptor open; any other mode is CPython's `invalid FileType('b') value: '-'`), a file that cannot be opened is the argparse error CPython prints (`argument files: can't open 'x': ...`, exit 2; a ValueError from the open — an invalid mode — is `invalid FileType(mode) value: ...`); an update mode (`FileType("rb+")`) is refused at conversion; `FileType` is recognized through the symbol table only — `argparse.FileType` or the name imported from argparse — and any other `type=` callable is loud; `action="version"` and `version=` are required together in either keyword order (`version=` with another action is Python's TypeError, loud); the parser is read from the body's top-level statements, and any reference to it under control flow or inside a nested definition is refused (the one statement visitor finds it); `nargs="+"` / `"*"` on ONE positional makes it a `Vec` field (the other positionals take one token each from either side, as CPython's pattern match does); `dest=` names an option's field and metavar; `action="store"` is the default action and `store_true` accepts `default=False`; an option's `default=` is kept as CPython keeps it — `type=` applies to strings only — so a typed option takes a literal of its own type or a string literal converted at conversion time as the parser would (`type=int, default="3"` is 3), and a non-string default of another type (`type=float, default=1`, the int 1 in CPython) or a string the type rejects is refused; a string default and a command-line value are read through Python's numeric-string grammar (`1_000` and `1_0.5`; `_1`, `1_` and `1__2` are CPython's invalid-value error), the runtime's one authority for `int(s)`/`float(s)` as well, which the converter mirrors for conversion-time defaults; a str option's non-literal `default=` is evaluated WHERE THE add_argument STATEMENT STOOD, like a version string, so a name rebound before parse_args does not change it; `action="version"` with `version=` (any str expression, evaluated WHERE THE add_argument STATEMENT STOOD — a name rebound before parse_args does not change it, as in Python) prints it and exits 0 with CPython's default help text, and has no field; the runtime parser is a port of CPython's `_parse_known_args`: tokens classified as argument/option/`--`, positionals consumed at each option boundary through the partial pattern match (`-*A-*`, `-*[A-]*`, `-*A[A-]*`), options through `consume_optional` (`--opt=v`, `-s2.5`, `-vn 3` packed flags, `--version=1` is `ignored explicit argument '1'`), every action taken in ARGV ORDER (a positional that cannot open fails before a later option; `-h`/`--version` print where they are consumed), option names in errors as CPython spells them (`-n/--num`), required positionals reported before leftovers; the usage line, the description and every help string wrap at the terminal width exactly as CPython's HelpFormatter does (COLUMNS when set, else the tty width of stdout, else 80, minus 2; the usage by parts with continuation lines aligned after the program name, the description through textwrap.fill, help strings whitespace-collapsed and wrapped at the help column `min(24, width - 20)`); every argparse spelling the converter reads (parser keywords, argument keywords, actions, type names, nargs) is a typed enum with a `from_name`, so an unknown one is a loud error naming it; `%(prog)s` is substituted in a version string and the description exactly when the text mentions it (`_format_text`; `100%` prints verbatim otherwise), and a help string is always `%`-formatted with `prog`, `default`, `dest` and `type` (`_expand_help`; `%(type)s` is the callable's name — `int`, `float`, `str` for an explicit `type=str`, `FileType('rb')` for a FileType, None without `type=`; a stray `%` or another key is the exception CPython raises, printed and exit 1); `version=` must be a str expression — CPython raises TypeError when `--version` runs with anything else, so a program whose version action can never print is refused at conversion; an option string that looks like a negative number (`-1`) makes every unmatched negative-number token an option, so `-2` is "unrecognized arguments" as in CPython; `parse_args(argv)` passes the explicit list (`list[str] \| None` through, a plain list wrapped). `nargs` on an option, `nargs="?"`, a second variadic positional, a positional `store_true` (CPython sets it True without consuming a token), an option string already registered (`argument -x/--two: conflicting option string: -x`, CPython's ArgumentError at add_argument), a repeated keyword on the constructor or on add_argument (CPython's `SyntaxError: keyword argument repeated`, which rython's front end does not raise itself), a FileType default and FileType's bufsize/encoding/errors stay loud | Correct (issue #332) |
| `issubclass(<computed value>, cls)`: `issubclass(type(x), C)` is `isinstance(x, C)` and lowers as that; a first operand rooted at an EXTERNAL module's value (`importlib.import_module(...).IncrementalDecoder`) is statically `false` — classes are not runtime values, so nothing is known about it — reported through the -W channel like `hasattr` over an unmodeled value; any other computed operand (a value, Python's TypeError; a class expression the conversion cannot resolve) and a NAME operand that is not a class are the loud refusal | External-object divergence, loud (issue #332) |
| `open(path, mode)` with a LITERAL mode containing 'b' is the bytes file — io.BytesIO's type over a disk backend — whose `read()` yields bytes and `write()` takes them; text modes stay the text file. The mode grammar is CPython's, checked in its order before any file-system side effect (`parse_open_mode`, one authority for `open` and the binary open): a letter outside `axrwb+t` or a repeated one is `invalid mode: '...'`, `t` with `b` is `can't have text and binary mode at once`, more than one of `a`/`x`/`r`/`w` or none are the two `exactly one of create/read/write/append mode` errors; `x` creates exclusively (FileExistsError); an update mode (`rb+`, `r+`) is refused at CONVERSION when the mode is a literal and loud at runtime otherwise; a computed binary mode reaching `open()` is loud (the bytes file type is chosen from a literal); in a binary mode an `encoding` or `errors` argument that is not None — by keyword or by position — raises CPython's `binary mode doesn't take an encoding argument` / `... an errors argument` at the open (after the mode grammar, before any file-system effect), a literal None is not given, an expression that may or may not be None is refused at conversion, and the same parameter by name and by position is Python's TypeError, refused at conversion. A write on a read-only stream or a read on a write-only one is `io.UnsupportedOperation` — an OSError and a ValueError, from the interpreter-derived tree, so `except OSError:` catches it — with CPython's messages (`write`/`read` for a binary file, `not writable`/`not readable` for a text file); an OSError's filename is Python's repr (an apostrophe switches the quotes), while argparse's own `can't open '...'` wrapper keeps its literal quotes as CPython's does; `getvalue()` on a disk handle is CPython's AttributeError naming the receiver (`'_io.BufferedReader' object has no attribute 'getvalue'`, BufferedWriter, TextIOWrapper). File objects are REFERENCES: a PyFile / bytes file is a cheap-clone handle over one shared stream (an `Rc<RefCell>`, the shared-instance scheme — a disk handle crossing a thread is refused by rustc; only the standard streams are process-wide), so `for f in files: f.close()` closes the caller's files and every method takes `&self` (a loop binding needs no `mut`). `f.name` is the opened path; `f.closed` is a method on the runtime and the attribute spelling does not compile yet (loud, E0615) | Correct (issue #332); `f.closed` attribute: loud |
| A name bound to the boxed PyValue (a parameter whose union annotation boxes — `body: _TYPE_BODY \| None` — an `Optional[alias]` that boxes, a value-pinned parameter) is never an Option slot: its `name = None` store, under any control flow, is the boxed None; a tuple-target None into a boxed name or a boxed field (`old_pool, self.pool = self.pool, None`) is the boxed None and the field it swaps out of is read by clone; a plain boxable argument (a bytes literal) into such a parameter boxes rather than Some-wraps, whatever the optional spelling; a module-level type-alias annotation resolves through the CALLEE's module at the call site, so an `Option<PyValue>` local unwraps into the boxed slot and a class instance stays a loud mismatch | Correct (issue #335) |
| A parameter annotated with a union of exception classes ONLY (`err: BaseSSLError \| OSError \| SocketTimeout`; `BaseException \| None` is `Option<PyException>`) is the runtime's one exception type, PyException — one rule for the signature, the body's name types, the call site's expected type and the alias resolver — so a caught exception passes in unchanged — in every union spelling (`A \| B \| None`, `Union[A, B]`, `typing.Union[A, B]`, `Optional[A]`, nested) through one member reader — the alias resolver applies it before resolving any member alone, an `except ... as e` name is typed as the exception (so it Some-wraps into an `Option[...]` slot), a quoted annotation is evaluated once, at the parser bridge that builds the parameter (so the signature, the body's name types, the local type pass, optional-name seeding, call sites and field inference all read the same expression), an alias cycle among union members ends the member walk as "not an exception class" (and the name inference's own path guard unwinds with its scope), a module-qualified member (`errors.MyError`) is judged through the crate's module authorities and the exception closure, or by the convention when the module is external, the `*Error`/`*Exception`/`*Warning` naming convention applies to a genuinely unresolved name only (an import from a module the crate does not hold) — a name bound as a value (`NetworkError = 1`), a bound function, a module import, an except binding, or an item imported from a stdlib or crate module is what it is whatever its spelling, while a binding or alias to another name is judged by what it names, a reused name passed into an Optional slot is cloned inside its `Some`, and a bare `import typing`/`typing_extensions` emits nothing at runtime like the from-import form (an aliased root, `import typing as t`, is rewritten to `typing` in every annotation when the module is built, on the shared statement walker, with bindings tracked per lexical scope and in source order: in the module and a class body an annotation sees the binding active at its point — an import, a conditional block's included, declares the alias from there on and any other binding of the name ends it — while a function body is its own scope, a name it binds anywhere being local throughout, and an alias imported in a nested scope never reaches the enclosing one; `typing_extensions.X` is read as `typing.X`) (a bare `import abc`/`contextlib`/`dataclasses` is dropped with a warning: their items are modeled under from-import names only); a class of the crate counts by the crate's one exception authority, the closure's `is_exception_class` (its C3 ancestry through the crate's classes, the builtin exceptions and the §8.1 naming convention), and an unresolvable external name follows the naming convention, as the raise model does; a union MIXING exception classes with boxable members stays the boxed PyValue, and an exception argument to it is a loud mismatch (the box holds no exception). `isinstance(x, SocketTimeout)` under `from socket import timeout as SocketTimeout` canonicalizes the alias to its builtin (`TimeoutError`) as the except clause does — on an except-bound name and on a PyException-typed parameter (it used to fold to false with a warning); `str(err)` on one is the exception's display, read through a reference | Correct (issue #335) |
| A field assigned a list comprehension takes its element expression's type, with the generators' targets typed by the comprehension scope (`m.strip()` over `modes.split(",")` is a String element): a call into a class-returning function is that class (the hierarchy's slot type for a polymorphic root, so `for d in reversed(self._decoders): data = d.decompress(data)` dispatches), a construction its class; an element the field inference cannot type stays the boxed PyValue. A method call on a construction in place (`Response(...)._decode(data, True)`) is typed by the method's declared return, so `print()` of a bytes result prints CPython's `b'...'`. A conditional expression returned from a scalar-typed function (`-> str`) or stored into a scalar slot renders each arm against the slot's type (a literal arm owns itself, a boxed slot boxes both) | Correct (issue #335) |
| Conversion time is linear in a module's size for its data tables: the symbol scopes share their bindings on clone (copy-on-write at the first store), so a module carrying its own tables (idna's uts46data.py — eighty functions of a hundred tuples each) converts in seconds; the per-element typed re-render used to copy the whole module per element and never finished. rypip's dependency cache extracts every artifact into its own directory (`extracted/<artifact stem>/`) atomically — unpacked into a sibling, validated, marked complete (`.rypip-complete`, naming the artifact), renamed in (the sibling is unique per invocation, threads included; every failure after it exists removes it; a sibling older than an hour is a crashed run's and is cleared; an incomplete tree at the path is set aside by rename and removed only if it is still incomplete once aside, so a tree another resolver published meanwhile is never deleted, and a concurrent resolver that published the same artifact first wins the final rename; a leftover sibling is cleared only when its owning process, named in the sibling, is provably dead — age is no evidence of liveness; the downloaded artifact and its digest sidecar are published whole through a unique temporary and a rename, so a concurrent reader never sees a truncated file; a cached version keeps its epoch and its PEP 440 local label (`1.0+cpu`; a specifier without a label matches the local variant); a wildcard specifier (`==1.0.*`) matches the release prefix with any pre/post/dev marker it spells matched exactly, ignoring local labels, and a non-version prefix matches nothing; a cache written before the markers and sidecars existed is not trusted offline (nothing verifies it) — one online resolution re-verifies the artifact in place without downloading it again (the file is hashed against the index, the sidecar written) and rebuilds its extraction from the local file; the online path never removes an extraction in place, the digest-tied extraction sets a stale tree aside under the rename-and-recheck protocol; a corrupt cached candidate (its bytes no longer hash to its sidecar) is the loud error when the ranking would have chosen it — never a silent downgrade — and is skipped when a candidate it would not have outranked resolves; the completion marker records the artifact's digest, and an extraction is a cache candidate only for the present, verified artifact of that digest — a replaced artifact rebuilds it; the extraction reads the artifact once, verifies those bytes against the sidecar (a torn pair is a loud mismatch the online path heals by publishing its verified pair again) and unpacks them; at one version a pure-Python wheel beats an sdist whatever was extracted so far, a native wheel is never a candidate (the online eligibility), and per artifact its complete extraction beats the unextracted artifact, in any directory order) — so two cached versions of one distribution never merge and an interrupted extraction is never a cache hit; a verified download records its sha256 beside the artifact, and the offline path reuses an artifact only through that digest (a mismatch is a loud error naming the file; an online resolution re-checks a cached file against the index's digest and replaces a stale one with its extraction); among several cached versions satisfying a requirement the newest is chosen, as online; an sdist stem's version is everything after the distribution name (`pkg-1.0-rc1` is 1.0rc1), a wheel's the one component before its tags | Correct (issue #335; Devin review on #342) |
| `issubclass(C1, C2)` folds through C1's chain as the raise attaches it — C1 resolved locally or through its import, every user base (a neutral-named one included), then EVERY builtin of the chain's interpreter MRO (`C(A, B)` over `ZeroDivisionError` and `LookupError` is a subclass of `ArithmeticError` through A's branch); a chain that leaves the crate before a builtin is a conversion error. The builtin exception hierarchy (`issubclass` folds, alias canonicalization) comes from the live interpreter; a `python3` that cannot be run is a conversion error, never an empty hierarchy | Correct-or-loud (Devin review on #330) |
...
| A USER exception class derived from a BUILTIN (`class MyError(ValueError)`) is caught by `except ValueError:` — the ancestor chain is class metadata attached to every construction (a modeled `__init__`, none, a bare `raise MyError`), and both handler paths walk it through the builtin's MRO and alias spellings (`except EnvironmentError:` on a subclass of OSError) | Correct (the evaluation on issue #137; Devin review on #330) |
| An in-crate exception's constructor binds the raise's arguments as CPython binds a call (positionals in order, keywords by name to positional or keyword-only parameters, missing parameters to their defaults); each evaluated argument is bound once, in source order, and a default is modeled only when it is a constant; a starred or splatted argument, an unknown or duplicate keyword, a missing parameter, or a non-constant default is a loud rustc error at the raise site | Correct-or-loud (Devin review on #330) |
| `issubclass(C1, C2)` on exception classes folds through the interpreter's own MRO for builtins (`issubclass(FileNotFoundError, OSError)` is True) and the symbol table for user classes; a name that is neither is a loud conversion error | Correct-or-loud (the evaluation on issue #137; replaces a hand-typed parent map) |
| A modeled exception field whose __init__ parameter is unannotated is a loud conversion error at the read (`e.field`), naming the annotation to add | Correct-or-loud (the evaluation on issue #137) |
| A SHARED class's ORDERING with a REFLECTED comparison when the first __lt__ returns NotImplemented is not modeled (`==` is: see the `__eq__` row), and ref_cmp's Greater case calls the RIGHT operand's __lt__ (a pure total-order __lt__ makes the two calls observationally identical to CPython's sort) | Model limit of the shared-equality round (#329) |
...
| `return NotImplemented` in a METHOD named `__eq__` is CPython's equality dispatch, decided by the other parameter's lowering: a shared class's `object`/own-class other is the PyRef class, the method then returns `Option<bool>` (NotImplemented is its None) and the `==` boundary tries the left operand, the right one reflected, then identity (`a == b` with a declining and b deciding is b's answer; `t == t` True, two declining instances False); a boxed `object` other of a value class is never the same object (`false`); a value class compared with its own kind has no identity to ask — a loud rustc error. In any other function (a free function named `__eq__` included) it is a loud rustc error at the site (`compile_error!` naming the construct — `__ne__`'s identity default and an ordering or arithmetic dunder's reflected attempt are not modeled) | Correct-or-loud (the evaluation on issue #137) |
