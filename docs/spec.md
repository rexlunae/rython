...
| An exception __init__ with a statement beyond the `self.<field> = <param>` stores, the `super().__init__(<message>)` call, a docstring, or `pass` is a loud conversion error at the raise site (the statement would not run there) | Correct-or-loud (the evaluation on issue #137) |
...
| `return NotImplemented` lowers to the identity fallback (`false`) in `__eq__` only; in any other function it is a loud conversion error (`__ne__`'s identity default and an ordering dunder's TypeError are not modeled) | Correct-or-loud (the evaluation on issue #137) |
