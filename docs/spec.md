...
| The exception-attribute model binds a raise argument that contains a call to a temporary once, in order, and the message and each attr read the temporary; a pure argument (a name, a constant, a field read, arithmetic) renders in place | Correct; CPython's single evaluation (the evaluation on issue #137) |
...
| A USER exception class derived from a BUILTIN (`class MyError(ValueError)`) is caught by `except ValueError:` — the handler's discriminant fast path consults the recorded user ancestor chain | Correct (the evaluation on issue #137) |
