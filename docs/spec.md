...
| A zero-parameter `__init__(self)` is modeled like any other (a fixed message; an argument at the raise is the arity error); a variadic `__init__(self, *args, **kwargs)` is modeled when its body only forwards to `super().__init__(*args)` (idna's IDNAError), the message being the raise's one positional argument or empty — a body doing more, a keyword, or two positional arguments are loud. The last `self.<field> = <param>` store of a field wins, as in Python; a second `super().__init__` call (which would replace the args) is loud. A bare-name argument is captured, in source order, before a LATER argument that runs code (`E(counter, bump())` reads counter before bump rebinds it) | Correct-or-loud (Devin review on #330) |
...
| The builtin exception hierarchy (`issubclass` folds, alias canonicalization) comes from the live interpreter; a `python3` that cannot be run is a conversion error, never an empty hierarchy | Correct-or-loud (Devin review on #330) |
