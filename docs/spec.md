...
| A parameter annotated with a union of exception classes ONLY (`err: BaseSSLError \| OSError \| SocketTimeout`; `BaseException \| None` is `Option<PyException>`) is the runtime's one exception type, PyException — one rule for the signature, the body's name types, the call site's expected type and the alias resolver — so a caught exception passes in unchanged (a class of the crate counts by its ancestry — a base that is a builtin exception, a stdlib alias, or a crate class extending one — never by its name, so a value class called `ParseError` is not one; an unresolvable external name follows the exception naming convention, as the raise model does); a union MIXING exception classes with boxable members stays the boxed PyValue, and an exception argument to it is a loud mismatch (the box holds no exception). `isinstance(x, SocketTimeout)` under `from socket import timeout as SocketTimeout` canonicalizes the alias to its builtin (`TimeoutError`) as the except clause does — on an except-bound name and on a PyException-typed parameter (it used to fold to false with a warning); `str(err)` on one is the exception's display, read through a reference | Correct (issue #335) |
