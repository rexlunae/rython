...
| A module's body runs where Python runs it — at the IMPORT SITE, once: every crate module's `__module_init__` is guarded by Python's per-module import lock — NOT STARTED → RUNNING (owner thread) → DONE. An import of a crate module — at module level (in the importer's init, in statement order), under module-level control flow (the `use` is hoisted to module scope; the init calls sit where the branch runs), or inside a function (at the call site, on first execution) — calls the loaded modules' inits in Python's order (`import a.b`: the package `a`, then `a.b`; `from .a import x`: `a`, then `x` when it is a submodule). The owning thread re-entering (an import cycle) returns at once and continues with the module's partial state, as CPython's partially-initialized module; another thread waits until the body has finished; a body that raises leaves the module NOT STARTED, so a later import runs it again (CPython drops a failed import). A module imported only inside a never-called function never runs. The entry's `main` runs its own init. The package root `__init__` is the lib root, which the bin crate does not contain: what it runs at import time (statements, the crate modules its imports load, computed assignments) does not run for the binary, and the converter warns | Correct (issue #333, Devin review on #336/#338); root `__init__`: loud |
