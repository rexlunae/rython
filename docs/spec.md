...
| Exception-ness is TRANSITIVE through the crate's emitted classes (`Root(Exception)`, `Mid(Root)`, `LeafError(Mid)`: `Mid` is an exception class whatever its name — a marker, no trait machinery — and LeafError's ancestor chain records Mid, so `except Mid:` catches it); the closure is one crate-wide index installed like the hierarchy roots | Correct (Devin review on #330) |
...
| The entry module's `main` runs every crate module's body at startup — each sibling's `__module_init__`, in dependency order (a module's crate imports before the module, each once, a cycle broken at the module already entered — the order Python runs module bodies when the entry is imported), then its own. Python runs a module's top-level statements when the module is first imported; the crate has no import step, so a function-local import's module runs at startup too (the same eager model as the crate's lazily-initialized statics). A `main` that needed no wrapper (a bare `main()` call pattern) gets one when a sibling has a body to run. The package root `__init__` is the lib root, which the bin crate does not contain: its module-level statements do not run for the binary, and the converter warns when it has any (imports, defs, classes, docstrings and plain assignments are items, not statements) | Correct (issue #333); root `__init__` statements: loud |
