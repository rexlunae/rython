...
| `test_math` | BLOCKED | `__float__`-coercion FIXED (`math.ceil(FloatLike(...))`, heterogeneous `[float, FloatLike]` → `Vec<f64>`), and `math.fsum`/`sumprod`/`isqrt`/`cbrt`/`fma`/`hypot`/`nextafter`/`ulp`/`log`(x[,base]) + int→f64 arg coercion + exact-int ceil/floor/trunc all landed. test_math now advances past the log region to **line 1224** — `self.assertRaises(TypeError, math.log)` in `testLog`: a deliberate bad-arity call rython correctly refuses loudly at conversion (its runtime requires `log(x, base)`), which the unittest-harness (issue #334) would need to call-and-catch; then the ~1319 strict-`zip` wall and the ~1366 `sumprod([10, BadMultiply(), 30], ...)` heterogeneous `int`+non-float-class list await |
