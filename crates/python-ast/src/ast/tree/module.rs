...
use crate::{
    ASYNC_RUNTIME_FEATURE, CodeGen, CodeGenContext, CrossModuleClasses, CrossModuleMutSelf,
    ExprType, ModuleClassInfo, Name, Object, PythonOptions, Statement, StatementType,
    ...
    fn to_rust(
        ...
        if let Some(docstring) = self.get_module_docstring() {
            ...
            if self.raw.body.len() > 1 || self.looks_like_module_docstring() {
                ...
                let generated_comment = format!("Generated from Python file: {}", module_filename);
            ...
            if let crate::StatementType::Assign(a) = &s.statement {
                ...
                if let Some(names) = assign_name_targets(a) {
                    ...
                    if names
                        ...
                        && (const_static_type(&a.value).is_some() || is_type_alias_value(&a.value))
                    {
                        continue;
                    }
        ...
        let (init_hoisted, init_leaked) =
            hoisted_name_set(&module_init_raw, &ctx, &symbols, &options);
        let (main_hoisted, main_leaked) =
            hoisted_name_set(&main_body_raw, &ctx, &symbols, &options);
        ...
        crate::check_call_sites(&module_init_raw, &symbols, &options.name_types, &options)?;
        let main_info =
            crate::analyze_function_types(&main_body_raw, Some(&options), Some(&symbols));
        crate::check_aliasing(
            &main_body_raw,
            &symbols,
        ...
        crate::check_call_sites(&main_body_raw, &symbols, &main_info.name_types, &options)?;
        ...
        for (stmt_index, s) in self.raw.body.into_iter().enumerate() {
            ...
            if let crate::StatementType::Assign(a) = &s.statement {
                ...
                if let [crate::ExprType::Name(target)] = a.targets.as_slice()
                    && let Some(ty) = crate::ast::tree::assign::builtin_scalar_alias_type(&a.value)
                {
                ...
                if let Some(synth) = crate::try_lru_cache_factory(a, Some(&options), &symbols) {
                    stream.extend(
                        synth
                            .clone()
                            .to_rust(ctx.clone(), options.clone(), symbols.clone())
                ...
                if let Some(names) = assign_name_targets(a) {
                    ...
                    if names.len() == 1 && is_type_alias_value(&a.value) {
                        if let Some(ty) =
            ...
            if let crate::StatementType::Assign(a) = &s.statement {
                ...
                if promoted.is_empty() {
                    ...
                    for n in promoted {
                        ...
                        // type matches.
                        let (ty, wrapped) = match module_init_static_ty(&n, &a.value, &options) {
                ...
                crate::StatementType::Try(t) => {
                    !t.handlers.is_empty()
                        ...
                        && t.handlers.iter().all(|h| {
                            crate::ast::tree::try_stmt::is_bare_import_error(&h.exception_type)
                                && (matches!(h.exception_type, Some(crate::ExprType::Name(_)))
            ...
            if flattenable_try {
                if let crate::StatementType::Try(t) = &s.statement {
                    ...
                    for body_stmt in &t.body {
                        ...
                        if let crate::StatementType::ImportFrom(i) = &mut body_stmt.statement {
                                ...
                                && crate::ast::tree::import::is_stdpython_module(&root)
                            {
                                for (name, bound) in &dropped {
                                    if crate::ast::tree::import::stdpython_module_item(&root, name)
                                    {
                        ...
                        let body_is_decl = Self::is_declaration_statement(&body_stmt.statement);
        ...
        let init_hoist_skip: std::collections::HashSet<String> = promoted_statics
            ...
            .collect();
        let init_decls =
            hoisted_declarations(&module_init_raw, &ctx, &symbols, &options, &init_hoist_skip);
        if !init_decls.is_empty() {
            module_init_stmts.insert(0, init_decls);
        }
        let main_decls = hoisted_declarations(
            &main_body_raw,
            &ctx,
            &symbols,
            &options,
            ...
            if is_simple_main_call_pattern {
                ...
                if user_main_is_async {
                    ...
                    let attr_tokens: proc_macro2::TokenStream =
                        runtime_attr.parse().unwrap_or_else(|_| quote!(tokio::main)); // fallback to tokio::main

                    // Replace the user's function signature and add the
                    // feature-gated attribute.
                    let new_stream_str = stream_str.replace(
                        "pub async fn main (",
                        &format!(
                            "#[cfg_attr(feature = \"{}\", {})] async fn main(",
                            ASYNC_RUNTIME_FEATURE, runtime_attr
                        ),
                    );
                    stream = new_stream_str
                    ...
                    if has_module_init_code {
                        ...
                        let renamed_stream_str =
                            Self::rename_main_function_and_references(&stream_str);
                        stream = renamed_stream_str
                            .parse::<proc_macro2::TokenStream>()
                            .unwrap_or_else(|_| stream);
                ...
                let stream_str = stream.to_string();
                let has_python_main = stream_str.contains("pub fn main (")
                    || stream_str.contains("pub async fn main (");
                ...
                if has_python_main {
                    ...
                    for stmt in &mut main_body_stmts {
                        ...
                        if updated_stmt_str != stmt_str {
                            if let Ok(new_stmt) =
                                updated_stmt_str.parse::<proc_macro2::TokenStream>()
                            {
                                *stmt = new_stmt;
                            }
                        }
        ...
        match &stmt.statement {
            ...
            crate::StatementType::Try(t) => {
                t.body.iter().any(stmt_contains_async)
                    || t.handlers
                        .iter()
                        .any(|h| h.body.iter().any(stmt_contains_async))
            ...
            crate::StatementType::Assign(a) => {
                expr_contains_async(&a.value) || a.targets.iter().any(expr_contains_async)
            }
            ...
            crate::StatementType::Assert { test, msg } => {
                expr_contains_async(test) || msg.as_deref().is_some_and(expr_contains_async)
            }
            _ => false,
        }
    }
...
fn single_assign_name(stmts: &[crate::Statement]) -> Option<String> {
    ...
    match &stmts[0].statement {
        crate::StatementType::Assign(a) if a.targets.len() == 1 => match &a.targets[0] {
            crate::ExprType::Name(n) => Some(n.id.clone()),
            _ => None,
        },
        _ => None,
...
pub(crate) fn module_promoted_static_names(
    ...
    let symbols = (**module)
        .clone()
        .find_symbols(crate::SymbolTableScopes::new());
    let mut names = std::collections::HashSet::new();
...
fn module_package_path_from_defs(path: &[String], options: &PythonOptions) -> Vec<String> {
...
pub(crate) fn fold_static_import_trys(
    ...
    fn collect_imports<'a>(stmts: &'a [crate::Statement], out: &mut Vec<&'a crate::StatementType>) {
        for s in stmts {
            match &s.statement {
                st @ (crate::StatementType::Import(_) | crate::StatementType::ImportFrom(_)) => {
                    out.push(st)
                }
                crate::StatementType::Try(t) => {
    ...
    for stmt in body {
        ...
        if let crate::StatementType::Try(t) = &stmt.statement
            ...
            && (t.handlers[0].exception_type.is_none()
                || crate::ast::tree::try_stmt::is_bare_import_error(&t.handlers[0].exception_type))
        {
...
fn module_function_free_reads(body: &[crate::Statement]) -> std::collections::HashSet<String> {
    ...
    all_names.difference(&bound).cloned().collect()
}
...
pub(crate) fn aliased_external_import(name: &str, options: &crate::PythonOptions) -> bool {
    let Some(module) = options.module_defs.get(&options.this_module_path) else {
        return false;
    };
    let module: &crate::Module = module;
    fn scan(body: &[crate::Statement], name: &str, options: &crate::PythonOptions) -> bool {
        use crate::StatementType as ST;
        for s in body {
            match &s.statement {
                ST::ImportFrom(i) => {
            ...
            match &s.statement {
                ST::Assign(a) => {
                    if a.targets
                        ...
                        && let Some(c) = crate::ast::tree::raise_stmt::stdlib_exception_canonical(
                            &m.id, &attr.attr,
                        )
                    {
                        return Some(c);
...
fn module_reexports_item(
    ...
    for s in &module.raw.body {
        ...
        if let ST::Import(im) = &s.statement {
            if im.names.iter().any(|a| {
                ...
                a.name
                    .split('.')
                    .next()
                    .is_some_and(crate::is_stdpython_module)
            }) {
                return true;
            }
            continue;
        }
        let ST::ImportFrom(i) = &s.statement else {
            continue;
        };
        ...
        if !i
            .names
            .iter()
            .any(|a| a.asname.as_deref() == Some(name) || (a.asname.is_none() && a.name == name))
        {
            continue;
        }
...
fn scan_module_body_for_item(options: &crate::PythonOptions, path: &[String], name: &str) -> bool {
    let Some(module) = options.module_defs.get(path) else {
        return false;
    };
    let module: &crate::Module = module;
...
pub(crate) fn const_static_type(value: &crate::ExprType) -> Option<TokenStream> {
    match value {
        ...
        crate::ExprType::BinOp(op) => {
            ...
            match (const_static_type(&op.left), const_static_type(&op.right)) {
...
pub(crate) fn type_contains_uninferred(t: &crate::TypeInfo) -> bool {
    match t {
        ...
        | crate::TypeInfo::Borrowed(inner) => type_contains_uninferred(inner),
...
fn wrap_module_error(filename: &str, e: Box<dyn std::error::Error>) -> Box<dyn std::error::Error> {
...
impl Module {
    ...
    fn is_simple_main_call_block(body: &[crate::Statement]) -> bool {
        ...
        match &stmt.statement {
            ...
            crate::StatementType::Expr(expr) => Self::is_main_function_call(&expr.value),
    ...
    fn is_main_function_call(expr: &crate::ExprType) -> bool {
        match expr {
            crate::ExprType::Call(call) => match call.func.as_ref() {
    ...
    fn is_simple_expression(expr: &crate::ExprType) -> bool {
        ...
        match expr {
            ...
            Call(_) | BinOp(_) | Compare(_) | BoolOp(_) | IfExp(_) | Dict(_) | Set(_) | List(_)
            | Tuple(_) | ListComp(_) | Lambda(_) | Attribute(_) | Subscript(_) | Starred(_)
            | DictComp(_) | SetComp(_) | GeneratorExp(_) | Await(_) | Yield(_) | YieldFrom(_)
            | FormattedValue(_) | JoinedStr(_) | NamedExpr(_) => false,
    ...
    fn update_main_references(code: &str) -> String {
        ...
        result = method_pattern
            .replace_all(&result, ".call_python_main(")
            .to_string();
        ...
        result = assignment_pattern
            .replace_all(&result, "= python_main")
            .to_string();

        // Pattern 4: Handle return statements like "return main"
        let return_pattern = Regex::new(r"return\s+main\b").unwrap();
        result = return_pattern
            .replace_all(&result, "return python_main")
            .to_string();
...
pub(crate) fn class_subclassed_crate_wide(name: &str, options: &crate::PythonOptions) -> bool {
    ...
    fn imports_name_from(stmts: &[crate::Statement], name: &str, leaf: &str) -> bool {
        ...
        stmts.iter().any(|s| match &s.statement {
            ...
            ST::If(b) => {
                imports_name_from(&b.body, name, leaf) || imports_name_from(&b.orelse, name, leaf)
            }
            ST::Try(t) => {
                imports_name_from(&t.body, name, leaf)
                    || t.handlers
                        .iter()
                        .any(|h| imports_name_from(&h.body, name, leaf))
            }
            _ => false,
        })
    }
        ...
        Some(crate::SymbolTableNode::ImportFrom(i)) => {
            ...
            if options.module_defs.contains_key(&path) {
                let canonical = i
                    ...
                    .map(|a| a.name.clone())
                    .unwrap_or_else(|| name.to_string());
                resolve_imported_class(options, &path, &canonical, 0).map(|(c, _)| c)
            } else {
                None
            }
        }
...
/// module.
pub fn module_defs_key<'a>(options: &'a PythonOptions, path: &'a [String]) -> Option<&'a [String]> {
    if options.module_defs.contains_key(path) {
        Some(path)
    } else if path.len() > 1
...
pub fn module_function_def(
    ...
    for s in &module.raw.body {
        if let crate::StatementType::FunctionDef(f) = &s.statement {
            if f.name == name {
                let is_overload_stub = f.decorator_list.iter().any(|d| match d {
...
impl CrossModuleClasses {
    ...
    fn computed_class_info(&self, path: &[String]) -> Option<&std::rc::Rc<ModuleClassInfo>> {
        match self {
...
/// transpiles.
pub fn module_widens_method_cached(options: &PythonOptions, root_name: &str, method: &str) -> bool {
    {
...
pub fn cross_module_mut_self_table(
    ...
    let mut table = std::collections::HashMap::<String, std::collections::HashSet<String>>::new();
