...
fn test_python_builtin_functions() {
    ...
    assert_eq!(
        range_start_stop_step(0, 10, 2).unwrap().collect::<Vec<_>>(),
        vec![0, 2, 4, 6, 8]
    );
...
fn test_complex_python_simulation() {
    ...
    fn process_text(text: &str) -> PyDictionary<String, String> {
        ...
        result.set(
            "uppercase".to_string(),
            py_text.upper().as_str().to_string(),
        );
    ...
    assert_eq!(
        text_result.get(&"word_count".to_string()),
        Some(&"3".to_string())
    );
    assert_eq!(
        text_result.get(&"uppercase".to_string()),
        Some(&"HELLO WORLD PYTHON".to_string())
    );
