...
use crate::{AsPathLike, PyException};
use std::fs;
use std::path::Path;
...
impl EnvBuilder {
    ...
    pub fn create<P: AsPathLike>(&self, env_dir: P) -> Result<(), PyException> {
        ...
        fs::create_dir_all(&bin_path)
            .map_err(|e| crate::runtime_error(format!("Failed to create bin directory: {}", e)))?;

        let lib_dir = env_path.join("lib");
        fs::create_dir_all(&lib_dir)
            .map_err(|e| crate::runtime_error(format!("Failed to create lib directory: {}", e)))?;
        ...
        fs::write(&pyvenv_cfg, cfg_content)
            .map_err(|e| crate::runtime_error(format!("Failed to create pyvenv.cfg: {}", e)))?;
        ...
        .map_err(|e| crate::runtime_error(format!("Failed to create python executable: {}", e)))?;
        ...
        #[cfg(unix)]
        {
            ...
            let mut perms = fs::metadata(&python_exe)
                .map_err(|e| {
                    crate::runtime_error(format!("Failed to get file permissions: {}", e))
                })?
                .permissions();
            perms.set_mode(0o755);
            fs::set_permissions(&python_exe, perms).map_err(|e| {
                crate::runtime_error(format!("Failed to set executable permissions: {}", e))
            })?;
        }
