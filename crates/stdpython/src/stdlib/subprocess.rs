...
use crate::{AsEnvLike, AsPathLike, AsStrLike, PyException, python_function};
...
pub fn run_with_output<A: AsRef<str>, C: AsRef<str>>(
    args: Vec<A>,
    cwd: Option<C>,
    capture_output: bool,
) -> Result<CompletedProcess, PyException> {
    ...
    if capture_output {
        ...
        match command.output() {
            Ok(output) => {
                ...
                Ok(CompletedProcess::with_output(
                    args_owned, returncode, stdout, stderr,
                ))
            }
            Err(e) => Err(crate::runtime_error(format!(
                "Failed to execute command '{}': {}",
