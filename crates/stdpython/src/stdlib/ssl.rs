//! Python ssl module — CLIENT-side TLS over the rustls crate (feature
//! `ssl-rustls`, ON by default; disable with default-features = false).
//!
//! What is modeled: the SSLContext surface the top converted packages
//! (urllib3/requests) drive — construction, verify_mode/check_hostname,
//! min/max TLS versions, CA loading (system roots via webpki-roots, PEM
//! files/data via rustls-pemfile), ALPN, and wrap_socket() performing
//! the handshake over a connected `socket.Socket` (the TLS session
//! shares the same descriptor, exactly like Python wrapping the fd).
//!
...
//!   renegotiates, and only speaks TLS 1.2/1.3 — a superset of what the
//!   usual OP_NO_* hardening asks for). Only minimum_version /
...
/// "OpenSSL ..." string: version-sniffing code (urllib3's ssl_.py)
/// then takes its generic path instead of applying OpenSSL-specific
/// workarounds that do not exist here.
pub const OPENSSL_VERSION: &str = concat!("rustls ", env!("CARGO_PKG_VERSION"));
pub const OPENSSL_VERSION_NUMBER: i64 = 0;
/// CPython's is a 5-tuple; comparisons in the wild are against 3-tuples
/// (`ssl.OPENSSL_VERSION_INFO < (1, 1, 1)` — urllib3's __init__), and
/// rython tuples compare same-arity only, so this is the 3-prefix.
/// All zeros: rustls is not OpenSSL (a documented divergence).
pub const OPENSSL_VERSION_INFO: (i64, i64, i64) = (0, 0, 0);
...
/// for TLS-in-TLS. Not modeled: rustls has no BIO-style engine surface
/// here, and `wrap_bio` itself is absent (urllib3's SSLTransport already
/// raises ProxySchemeUnsupported on that check). The construction raises
/// a catchable NotImplementedError instead of handing back a silently
/// inert buffer (documented divergence).
#[allow(non_snake_case)]
pub fn MemoryBIO() -> Result<crate::PyValue, PyException> {
    Err(PyException::new(
        "NotImplementedError",
