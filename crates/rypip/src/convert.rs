...
fn generate_kernel_lib_rs(source: &str) -> Result<String> {
    ...
    let modinfo_len = license.len() + 8; // "license="
    let padded_len = (modinfo_len + 7) & !7; // align to 8 bytes
    ...
    // Module license as a .modinfo ELF section entry.
    out.push_str("#[no_mangle]\n#[link_section = \".modinfo\"]\n");
    out.push_str(&format!("static __mod_license: [u8; {padded_len}] = *b\"license={license}"));
    for _ in modinfo_len..padded_len {
        out.push_str("\\0");
    }
    out.push_str("\";\n\n");
...
fn write_cargo_toml(
    ...
    // Kernel modules are bare no_std with no runtime — no stdpython at all.
