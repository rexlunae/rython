...
/// The first line of every Cargo.toml rypip writes: how `rypip run` tells a
/// crate it generated (and may regenerate) from a directory that is not
/// its own.
pub const GENERATED_MANIFEST_HEADER: &str = "# Generated by rypip from a Python package. Edit freely.";

thread_local! {
    /// Every file the conversion on this thread has written so far, in
    /// order: the explicit ownership `rypip run` needs to know which files
    /// under `src/` are its own to replace (Devin review on #340, round 7).
    static WRITTEN: std::cell::RefCell<Vec<PathBuf>> = const { std::cell::RefCell::new(Vec::new()) };
}

/// The one way the converter writes a file: `fs::write`, recorded.
fn write_file(path: impl AsRef<Path>, contents: impl AsRef<[u8]>) -> std::io::Result<()> {
    fs::write(path.as_ref(), contents)?;
    WRITTEN.with(|w| w.borrow_mut().push(path.as_ref().to_path_buf()));
    Ok(())
}

/// The files written since the conversion began, for [`ConvertedCrate::written`].
fn take_written() -> Vec<PathBuf> {
...
pub struct ConvertedCrate {
    pub root: PathBuf,
    pub name: String,
    /// Every file this conversion wrote (Cargo.toml, the sources, a build
    /// script), absolute: what `rypip run` may replace on the next run,
    /// and nothing else.
...
fn convert_on_this_thread(
    ...
    if opts.kernel_module {
        ...
        write_file(src_dir.join("lib.rs"), &kernel.code)?;
        if let Some(device) = &kernel.device {
            write_file(src_dir.join("device.rs"), device)?;
        }
    ...
    for (module, code) in &transpiled {
        ...
        write_file(&file, format_rust(&contents))
    ...
    if let Some(bindings) = &bindings_text {
        write_file(src_dir.join("python_api.rs"), format_rust(bindings))?;
        let mut lib = fs::read_to_string(&lib_rs)?;
        lib.push_str("\n#[cfg(feature = \"python\")]\nmod python_api;\n");
        write_file(&lib_rs, format_rust(&lib))?;
    }
...
fn convert_driver(
    ...
    write_file(src_dir.join("lib.rs"), format_rust(&lib))?;
    ...
    let main = driver_glue_template()
        ...
        .replace("@@IOC_RESET@@", &format!("{:x}", manifest.ioc_reset))
        .replace("@@IOC_STATS@@", &format!("{:x}", manifest.ioc_stats));
    write_file(src_dir.join("main.rs"), format_rust(&main))?;
...
fn write_missing_containers(
    ...
    for path in children.keys() {
        ...
        write_file(&file, format_rust(&mod_decls(children, path, true)))
            .with_context(|| format!("writing {}", file.display()))?;
    }
    Ok(())
}
...
fn write_cargo_toml(
    ...
    let mut toml = format!(
        "{header}\n\
         ...
         [dependencies]\n\
         {stdpython_dep}\n",
        header = GENERATED_MANIFEST_HEADER,
        name = crate_name,
    ...
    write_file(out_dir.join("Cargo.toml"), toml)?;
...
"#;
    ...
    write_file(out_dir.join("Makefile"), &makefile)?;
    write_file(out_dir.join("pahole-wrapper"), wrapper)?;
